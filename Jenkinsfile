...
//            sh "echo '[kube]' > ansible_playbooks/kube.ini;"
            sh "${TERRAFORM_CMD} output -json gk_server_public_ips | jq '.value' | head -n -1 | tail -n +2 | sed 's/[,\"]//g' >> ansible_playbooks/all.ini"
            sh "echo '[kube-master]' >> ansible_playbooks/kube.ini; head -n1 ansible_playbooks/all.ini >> ansible_playbooks/kube.ini"
            sh "echo '[kube-node]' >> ansible_playbooks/kube.ini; tail -n +2 ansible_playbooks/all.ini >> ansible_playbooks/kube.ini"
    ...
    stage('ansible: kube node') {
        ansiblePlaybook(
            playbook: 'ansible_playbooks/node.yml',
            inventory: 'ansible_playbooks/kube.ini',
            credentialsId: '3276ccf3-13bc-4408-b815-7b07bfd4e972',
            becomeUser: 'centos',
            sudo: true)
